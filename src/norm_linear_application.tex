...
\real}$ une fonction.
    ...
    \begin{enumerate}
        ...
        \item $\forall \lambda \in \mathbb{K}$, $\forall x \in E$, $N(\lambda x)
            \leq |\lambda| N(x)$
    \end{enumerate}
    Cette application est souvent notée $\GSnormeDef{.}{E}$ ou tout simplement
    $\GSnorme{.}$ s'il n'y a pas de confusion sur l'espace de définition de $N$.

	On dit que le couple $(E, \GSnormeDef{.}{E})$ est un espace vectoriel normé.
\end{definition}
...
\begin{definition}
	...
	qu'elles sont équivalentes s'il existe $k_{1}, k_{2} \in \mathbb{K}$ tel
	que pour tout $x \in E$,
	\begin{equation}
		k_{1} \GSnormeDef{x}{1} \leq \GSnormeDef{x}{2} \leq
		k_{2} \GSnormeDef{x}{1}
	\end{equation}
\end{definition}
...
\begin{definition}
	...
	$\GSnormedSpace{E}{\GSnorme{.}}$ est un
	espace de Banach si E est complet pour $\GSnorme{.}$.
\end{definition}

\begin{exercice}
	Soit E un $\mathbb{K}$-espace vectoriel de dimension finie, N une norme
	quelconque sur E, et $\mathbb{K}$ complet.
	Montrez que $\GSnormedSpace{E}{N}$ est un espace de Banach.
...
Donc, en dimension finie, les applications inversibles forment un ensemble
